    ...
    try {
        ...
        const order = await prisma.$transaction(async (tx) => {
            ...
            for (const requestedItem of data.items) {
                const itemInventories = inventories.filter((inventory) => inventory.itemId === requestedItem.itemId);
                const availableQuantity = itemInventories.reduce((total, inventory) => total + inventory.physicalQuantity - inventory.reservedQuantity, 0);
            ...
            for (const requestedItem of data.items) {
                ...
                for (const inventory of itemInventories) {
                    ...
                    const quantityToReserve = Math.min(availableQuantity, remainingQuantity);
