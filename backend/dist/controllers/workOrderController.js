...
import { createWorkOrderSchema, updateWorkOrderStatusSchema, } from "../validators/workOrderValidator.js";
export const createWorkOrder = async (req, res) => {
    try {
        ...
        const { workOrderNumber, locationId, itemId, requiredQuantity, assignedUserId, } = result.data;
...
export const getWorkOrders = async (_req, res) => {
    try {
        ...
        const workOrdersWithShortage = await Promise.all(workOrders.map(async (workOrder) => {
            const inventory = await prisma.inventory.findFirst({
                where: {
            ...
            const availableQuantity = inventory
                ...
                : 0;
            const shortage = Math.max(workOrder.requiredQuantity - availableQuantity, 0);
            return {
                ...workOrder,
                availableQuantity,
                shortage,
...
export const updateWorkOrderStatus = async (req, res) => {
    try {
        ...
        const result = updateWorkOrderStatusSchema.safeParse(req.body);
        if (!result.success) {
            return res.status(400).json({
                success: false,
                message: "Invalid request data",
